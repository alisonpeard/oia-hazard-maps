...
import scripts.utils.stats as stats
...
if __name__ == "__main__":
    ...
    output_dir = f"../diagnoistics/hazard_detrended"
    ...
    beta0_scales = np.full(nq, np.nan)
    beta1_scales = np.full(nq, np.nan)
    ...
    for iq, quantile in enumerate(quantiles):
        ...
        exceed_years = years_centered[detrended > threshold]
        ...
        shapes[iq] = shape
        beta0_scales[iq] = beta0_scale
        beta1_scales[iq] = beta1_scale
        locs[iq] = threshold

        # get return levels 
        for i_epoch, epoch in enumerate(epochs):
            epoch_rescaled = epoch - years_valid.mean()
            expected = stats.mean_binom(epoch_rescaled, (beta0_trend, beta1_trend))
            for rp_idx, T in enumerate(rps):
                return_level = dgpd_quantile_nonstationary(1/T, beta0_scale, beta1_scale, shape, epoch_rescaled)
                return_level += expected + threshold
                return_levels[rp_idx, i_epoch, iq] = int(min(return_level, 365))
    # %%
    fig, axs = plt.subplots(1, 5, figsize=(12, 3), sharex=True)
    axs[0].plot(quantiles, locs)
    axs[1].plot(quantiles, beta0_scales)
    axs[2].plot(quantiles, beta1_scales)
    axs[3].plot(quantiles, shapes)
    axs[4].plot(quantiles, nexceeds)
    axs[0].set_title("Threshold (loc)")
    axs[1].set_title("β0 (scale)")
    axs[2].set_title("β1 (scale)")
    axs[3].set_title("Shape")
    axs[4].set_title("Number of exceedances")
    # %%
    cmap = plt.get_cmap("viridis_r")

    fig, axs = plt.subplots(1, len(epochs), figsize=(10, 5), sharex=True, sharey=True)
